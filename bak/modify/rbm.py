...
class RBM(object):
    def __init__(self,
        ...
        self.v1 = transfer_function(
            tf.matmul(self.h0prob, tf.transpose(self.rbm_w)) + self.rbm_vb)
        ...
        self.update_w = self.rbm_w + alpha * (
            self.w_positive_grad - self.w_negative_grad
        ) / tf.to_float(tf.shape(self.x)[0])
        self.update_vb = self.rbm_vb + alpha * tf.reduce_mean(
            self.x - self.v1, 0)
        self.update_hb = self.rbm_hb + alpha * tf.reduce_mean(
            self.h0prob - self.h1, 0)
    ...
    def _initialize_weights(self):
        ...
        with tf.variable_scope(self.scope_name):
            w = tf.Variable(
                tf.random_normal(
                    [self.n_input, self.n_hidden], stddev=0.01, dtype=tf.float32),
                name='rbm_w')
            vb = tf.Variable(
                tf.zeros([self.n_input], dtype=tf.float32), name='rbm_vb')
            hb = tf.Variable(
                tf.zeros([self.n_hidden], dtype=tf.float32), name='rbm_hb')

        all_weights = {'w': w, 'vb': vb, 'b': hb}
        self.saver = tf.train.Saver(all_weights)

        return all_weights

    def transform(self, batch_x):
        return self.sess.run(self.h_sample, {
            self.x: batch_x,
            self.rbm_w: self.o_w,
            self.rbm_vb: self.o_vb,
            self.rbm_hb: self.o_hb
    ...
    def partial_fit(self, batch_x):
        ...
        self.n_w, self.n_vb, self.n_hb = self.sess.run(
            [self.update_w, self.update_vb, self.update_hb],
            feed_dict={
                self.x: batch_x,
                self.rbm_w: self.o_w,
                self.rbm_vb: self.o_vb,
