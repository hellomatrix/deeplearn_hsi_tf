...
batch_size=100
hidden_layers_sizes=[60, 60, 60, 60] #四个隐层的神经网咯，初始化时每层单独训练。
corruption_levels = [0., 0., 0., 0.] #计算时，不用dropout
...
n_train_batches = datasets[0][0].get_value(borrow=True).shape[0] # trainset的样本数量
...
for i in xrange(sda.n_layers):
    ...
    for epoch in xrange(pretraining_epochs):
        ...
        for batch_index in xrange(n_train_batches):
            c.append(pretraining_fns[i](index=batch_index, # pretraining_fns[i]表示的是不同隐层的训练
